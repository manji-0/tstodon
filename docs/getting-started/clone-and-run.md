...
Local API authentication uses `Authorization: Bearer ${DEV_BEARER_SECRET}:${email}` (default secret `dev-secret`). Copy `.dev.vars.example` to `.dev.vars` before enabling Auth0-protected routes. Replace placeholder D1 / KV / R2 identifiers in `wrangler.jsonc` before a real deploy. See [Configuration](../reference/configuration.md) and [Local Core](../planning/local-core.md).
